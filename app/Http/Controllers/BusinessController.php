...
class BusinessController extends Controller
{
    ...
    public function services(string $slug)
    {
        ...
        if ($business) {
            if ($business->creator_id == Auth::id() && Auth::user()->role == 1) {
                return view('admin.business.services', ['slug' => $slug]);
            } else {
                return abort(401);
            }
        } else {
    ...
    public function contacts(string $slug)
    {
        $business = Business::where('slug', $slug)->where('status', 1)->first();
        if ($business) {
            if ($business->creator_id == Auth::id() && Auth::user()->role == 1) {
                return view('admin.business.contacts', ['slug' => $slug]);
            } else {
                return abort(401);
            }
        } else {
    ...
    public function settings(string $slug)
    {
        ...
        if ($business) {
            if ($business->creator_id == Auth::id() && Auth::user()->role == 1) {
                return view('admin.business.settings', ['slug' => $slug]);
            } else {
                return abort(401);
            }
        } else {
